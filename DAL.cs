...
namespace TP1
{
    internal class DAL
    {
        ...
        public List<Usuario> inicializarUsuarios()
        {
                ...
                new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    while (reader.Read())
                    {
                        aux = new Usuario(reader.GetString(0),reader.GetString(1), reader.GetString(2), reader.GetInt32(3), reader.GetString(4));
                        misUsuarios.Add(aux);
                    }
        ...
        public int agregarUsuario(int Dni, string Nombre, string Mail, string Password, bool EsADM, bool Bloqueado)
        {
            ...
            string queryString = 
                "INSERT INTO [dbo].[Usuarios] ([Dni],[Nombre],[Apellido],[Mail],[Password],[IntentosFallidos],[Bloqueado],[IdAmigos],[Id]) " +
                "VALUES (@dni,@nombre,@apellido,@mail,@password,@intentos, @bloqueado, @idAmigo, );";
            using (SqlConnection connection =
                new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(queryString, connection);
